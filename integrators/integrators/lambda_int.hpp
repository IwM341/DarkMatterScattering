...
/*
extern inline double integrateABCD(std::function<double(double,double)> f,double a,double b,double c,double d,int N1,int N2){
	
	double sum = 0.0;
	double sum5;
	double xi = 0;
	double h = (b-a)/N1;
	#pragma omp parallel for reduction(+:sum) private(sum5,xi) shared(N1,h)
	for (int i = 0; i < N1; i++) {
		sum5 = 0.0;
		xi = a + i*h;
		for(int j = 0;j<5;j++)
