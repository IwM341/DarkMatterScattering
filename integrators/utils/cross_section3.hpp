...
typedef std::function<double(PhaseState3 out,PhaseState2 in)> MatrixElementType23;
typedef std::function<double(PhaseState2 out,PhaseState2 in)> MatrixElementType22;
...
extern inline double sigmaC(double mp,double mk,double v_ls,double v_esc,
	...
	if(kt+ke <= k0){
		...
		*/
		...
		std::cout << "hard kinematics (kt+ke < k1p), easy calculation" <<std::endl;
		...
		double reg_sigma = integrateAB5([Ecm,P0,K0,M22,alpha,eps,mp,mk	,NTh,k0](double Th){
				...
				double factor = 1.+alpha/M_PI*log(eps/mp)*W(x);
